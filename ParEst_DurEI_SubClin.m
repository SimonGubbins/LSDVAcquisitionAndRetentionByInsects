function ParSamp=ParEst_DurEI_SubClin(D,nchains,nsamp,nburnin,nthin)
%
% ParSamp=ParEst_DurEISubClin(D,nchains,nsamp,nburnin,nthin)
%
...
% Set the initial point for sampling
    ...
    while ~isfinite(CurrL+prior)
        ...
        mu0=unifrnd(5,15);
        par=[k0; mu0];
        prior=log(exppdf(par(1),100))+log(exppdf(par(2),100));
...
% Compute the prior and log-likelihood
            prior_new=log(exppdf(par_new(1),100))+...
                      log(exppdf(par_new(2),100));
